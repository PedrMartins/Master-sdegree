...
colnames (dads.bio.cj)<- c("Amost","D","Alt","vol","Gen","Spp","Distri","Filo")
head(dads.bio.cj)
...
colnames (dads.bio.bc)<- c("Amost","D","Alt","vol","Gen","Spp","Distri","Filo")


dads.bio.it =  read.table ("BIOMASSA It.txt" , header = TRUE,
 sep = "\t", row.names=1, dec = ",")

colnames (dads.bio.it)<- c("Amost","D","Alt","vol","Gen","Spp","Distri","Filo")

dads.bio.fb =  read.table ("BIOMASSA fbD.txt" , header = TRUE,
 sep = "\t", row.names=1, dec = ",")

colnames (dads.bio.fb)<- c("Amost","D","Alt","vol","Gen","Spp","Distri","Filo")

dads.bio.fsf =  read.table ("BIOMASSA fsfD.txt" , header = TRUE,
 sep = "\t", row.names=1, dec = ",")

colnames (dads.bio.fsf)<- c("Amost","D","Alt","vol","Gen","Spp","Distri","Filo")

dads.bio.bp =  read.table ("BIOMASSA Bp.txt" , header = TRUE,
 sep = "\t", row.names=1, dec = ",")

colnames (dads.bio.bp)<- c("Amost","D","Alt","vol","Gen","Spp","Distri","Filo")
...
p.g.7[3],p.a.7[4],p.g.7[4]),ylab="Indivíduos (%)",
...
barplot (c(p.a.3[1],p.g.3[1],p.a.3[2],p.g.3[2],p.a.3[3],p.g.3[3],
		p.a.3[4],p.g.3[4]),xlab="Classe DAP (cm)",
...
barplot (c(b.p.a.3[1],b.p.g.3[1],b.p.a.3[2],b.p.g.3[2],b.p.a.3[3],b.p.g.3[3],
		b.p.a.3[4],b.p.g.3[4]), xlab="Classe DAP (cm)",
...
legend("topleft" #função adiciona um texto ao gráfico, 
			#arg 1º define a localização, usa-se a função locator para
			#adicionar de uma forma interativa
	,c("Angiosperma", "Gimnosperma") #texto a ser escrito
	,col=c("grey80","grey51")
...
jpeg(filename = "biomas_arau__podo_prop.jpg", width = 800, height = 950, # função salva gráficos em .jpg
     units = "px", quality = 75,
     bg = "white", restoreConsole = TRUE)

par(mfrow=c(2,1), mar=c(3,5,2,2), cex.axis=1.3, cex.lab=1.5, mgp=c(3.5,1.6,0),	
family="serif",las=1, tcl=0.3)
...
text(0.5,95 #função adiciona um texto ao gráfico, 
			#arg 1º define a localização, usa-se a função locator para
			#adicionar de uma forma interativa
...
bio.gim= as.matrix (bio.podo.ara)
barplot (bio.gim, col=c("grey31","grey77"), ylim=c(0,250),
 ylab=expression(Mg%.%ha^-1))


chisq.test (bio.gim[2,])
...
text(0.5,245 #função adiciona um texto ao gráfico, 
			#arg 1º define a localização, usa-se a função locator para
			#adicionar de uma forma interativa
...
colnames(bio.tem.trop) <- c("Campos do Jordão","Delfim Moreira \n Faz. São Fran.",
					"Delfim Moreira \n Faz. Bart.","Baependi",
					"Barra \n do Chapéu","Itaberá")
...
text(0.5,345 #função adiciona um texto ao gráfico, 
			#arg 1º define a localização, usa-se a função locator para
			#adicionar de uma forma interativa
	,"a" #texto a ser escrito
	,cex=1.5		#tamanho da fonte
...
text(0.5,115 #função adiciona um texto ao gráfico, 
			#arg 1º define a localização, usa-se a função locator para
			#adicionar de uma forma interativa
	,"b" #texto a ser escrito
	,cex=1.5		#tamanho da fonte
...
clas.ara.cj10<-dads.ara.cj [dads.ara.cj$DAP<10,]
clas.pod.cj10<-dads.pod.cj [dads.pod.cj$DAP<10,]
clas.antrop.cj10<-dads.antrop.cj [dads.antrop.cj$DAP<10,]
clas.antem.cj10<-dads.antem.cj [dads.antem.cj$DAP<10,]

ara.smal=length(clas.ara.cj10$DAP)
pod.smal=length(clas.pod.cj10$DAP)
antr.smal=length(clas.antrop.cj10$DAP)
ante.smal=length(clas.antem.cj10$DAP)

clas.ara.cj10.30<-dads.ara.cj [dads.ara.cj$DAP<10 & dads.ara.cj$DAP<30,]
clas.pod.cj10.30<-dads.pod.cj [dads.pod.cj$DAP<10 & dads.pod.cj$DAP<30,]
clas.antrop.cj10.30<-dads.antrop.cj [dads.antrop.cj$DAP<10 & dads.antrop.cj$DAP<30,]
clas.antem.cj10.30<-dads.antem.cj [dads.antem.cj$DAP<10 & dads.antem.cj$DAP<30,]

ara.med=length(clas.ara.cj10.30$DAP)
pod.med=length(clas.pod.cj10.30$DAP)
ante.med=length(clas.antem.cj10.30$DAP)
antr.med=length(clas.antrop.cj10.30$DAP)


clas.ara.cj30.50<-dads.ara.cj[dads.ara.cj$DAP>=30 & dads.ara.cj$DAP<50,]
clas.pod.cj30.50<-dads.pod.cj [dads.pod.cj$DAP>=30 & dads.pod.cj$DAP<50,]
clas.antrop.cj30.50<-dads.antrop.cj [dads.antrop.cj$DAP>=30 & dads.antrop.cj$DAP<50,]
clas.antem.cj30.50<-dads.antem.cj [dads.antem.cj$DAP>=30 & dads.antem.cj$DAP<50,]

ara.lar=length(clas.ara.cj30.50$DAP)
pod.lar=length(clas.pod.cj30.50$DAP)
ante.lar=length(clas.antem.cj30.50$DAP)
antr.lar=length(clas.antrop.cj30.50$DAP)


clas.ara.cj50<-dads.ara.cj [dads.ara.cj$DAP<50,]
clas.pod.cj50<-dads.pod.cj [dads.pod.cj$DAP<50,]
clas.antrop.cj50<-dads.antrop.cj [dads.antrop.cj$DAP<50,]
clas.antem.cj50<-dads.antem.cj [dads.antem.cj$DAP<50,]

ara.x.lar=length(clas.ara.cj50$DAP)
pod.x.lar=length(clas.pod.cj50$DAP)
antr.x.lar=length(clas.antrop.cj50$DAP)
ante.x.lar=length(clas.antem.cj50$DAP)

bio.tem.trop.pro = data.frame(
CJ_SPgim=c((ara.smal/(ara.smal+pod.smal))*100,(pod.smal/(ara.smal+pod.smal))*100),
CJ_SPang=c((ante.smal/(ante.smal+antr.smal))*100,(antr.smal/(ante.smal+antr.smal))*100)
)

CJ_SP1=c((ara.med/(ara.med+pod.med))*100,(pod.med/(ara.med+pod.med))*100,
(ante.med/(ante.med+antr.med))*100,(antr.med/(ante.med+antr.med))*100),
CJ_SP2=c((ara.lar/(ara.lar+pod.lar))*100,(pod.lar/(ara.lar+pod.lar))*100,
(ante.lar/(ante.lar+antr.lar))*100,(antr.lar/(ante.lar+antr.lar))*100),
CJ_SP3=c((ara.x.lar/(ara.x.lar+pod.x.lar))*100,(pod.x.lar/(ara.x.lar+pod.x.lar))*100,
(ante.x.lar/(ante.x.lar+antr.x.lar))*100,(antr.x.lar/(ante.x.lar+antr.x.lar))*100)
)

bio=as.matrix(bio.tem.trop.pro)
barplot (bio)
########################################################################################################
...
tail(bio.cj, 15)
b.g= bio.it[bio.it$Lvl.D=="genus",]
b.s= bio.it[bio.it$Lvl.D=="species",]
b.p= bio.it[bio.it$Lvl.D=="Ponderada",]
b.p= bio.it[bio.it$Lvl.D=="ponderada",]
